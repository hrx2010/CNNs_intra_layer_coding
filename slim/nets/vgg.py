...
def mapping_to_codebook_scalar_quant(x, st, base, num_levels):
  ...
  ts_st = tf.constant(st)
  ts_base = tf.constant(base)
  ts_num_levels = tf.constant(1.0 * (num_levels - 1))

  d = tf.subtract(x , ts_st)
  e = tf.divide(d , ts_base)
  f = tf.floor(e)
  g = tf.clip_by_value(f , clip_value_min = tf.constant(-1000000.0), clip_value_max = ts_num_levels)
...
def vgg_16_decomposed(inputs,
  ...
  with tf.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      net_conv1_before_relu = slim.conv2d(inputs , 64, [3, 3], activation_fn=None, scope='conv1/conv1_1')
      net_conv1 = tf.nn.relu(net_conv1_before_relu)
 
      net_conv2_before_relu = slim.conv2d(net_conv1 , 64, [3, 3], activation_fn=None, scope='conv1/conv1_2')
      net_conv2 = tf.nn.relu(net_conv2_before_relu)

      net_p_1 = slim.max_pool2d(net_conv2, [2, 2], scope='pool1')

      net_conv3_before_relu = slim.conv2d(net_p_1, 128, [3, 3], activation_fn=None, scope='conv2/conv2_1')
      net_conv3 = tf.nn.relu(net_conv3_before_relu)

      net_conv4_before_relu = slim.conv2d(net_conv3, 128, [3, 3], activation_fn=None, scope='conv2/conv2_2')
      net_conv4 = tf.nn.relu(net_conv4_before_relu)

      net_p_2 = slim.max_pool2d(net_conv4, [2, 2], scope='pool2')

      net_conv5_before_relu = slim.conv2d(net_p_2,  256, [3, 3], activation_fn=None, scope='conv3/conv3_1')
      net_conv5 = tf.nn.relu(net_conv5_before_relu)

      net_conv6_before_relu = slim.conv2d(net_conv5,  256, [3, 3], activation_fn=None, scope='conv3/conv3_2')
      net_conv6 = tf.nn.relu(net_conv6_before_relu)

      net_conv7_before_relu = slim.conv2d(net_conv6,  256, [3, 3], activation_fn=None, scope='conv3/conv3_3')
      net_conv7 = tf.nn.relu(net_conv7_before_relu)

      net_p_3 = slim.max_pool2d(net_conv7, [2, 2], scope='pool3')

      net_conv8_before_relu = slim.conv2d(net_p_3, 512, [3, 3], activation_fn=None, scope='conv4/conv4_1')
      net_conv8 = tf.nn.relu(net_conv8_before_relu)

      net_conv9_before_relu = slim.conv2d(net_conv8, 512, [3, 3], activation_fn=None, scope='conv4/conv4_2')
      net_conv9 = tf.nn.relu(net_conv9_before_relu)

      net_conv10_before_relu = slim.conv2d(net_conv9, 512, [3, 3], activation_fn=None, scope='conv4/conv4_3')
      net_conv10 = tf.nn.relu(net_conv10_before_relu)

      net_p_4 = slim.max_pool2d(net_conv10, [2, 2], scope='pool4')

      net_conv11_before_relu = slim.conv2d(net_p_4, 512, [3, 3], activation_fn=None, scope='conv5/conv5_1')
      net_conv11 = tf.nn.relu(net_conv11_before_relu)

      net_conv12_before_relu = slim.conv2d(net_conv11, 512, [3, 3], activation_fn=None, scope='conv5/conv5_2')
      net_conv12 = tf.nn.relu(net_conv12_before_relu)

      net_conv13_before_relu = slim.conv2d(net_conv12, 512, [3, 3], activation_fn=None, scope='conv5/conv5_3')
      net_conv13 = tf.nn.relu(net_conv13_before_relu)

      net_p_5 = slim.max_pool2d(net_conv13, [2, 2], scope='pool5')

      net_conv14_before_relu = slim.conv2d(net_p_5, 4096, [7, 7], activation_fn=None, padding=fc_conv_padding, scope='fc6')
      net_conv14 = tf.nn.relu(net_conv14_before_relu)

      net_conv14_drop = slim.dropout(net_conv14, dropout_keep_prob, is_training=is_training,
                         scope='dropout6')

      net_conv15_before_relu = slim.conv2d(net_conv14_drop, 4096, [1, 1], activation_fn=None, scope='fc7')
      net_conv15 = tf.nn.relu(net_conv15_before_relu)
