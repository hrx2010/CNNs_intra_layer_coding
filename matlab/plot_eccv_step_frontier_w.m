...
rate = 1:8
plot(kern_delta(rate+1,:,1)',log10(kern_W_sse(rate+1,:,1))'-2,'LineWidth',0.75,'Color',map(3,:));
hold on;
[kern_Y_min,i] = min(kern_W_sse(rate+1,:,1),[],2);
plot(kern_delta(1,i,1),log10(kern_Y_min)-2,'LineWidth',0.75,'Color',map(1,:));
plot(kern_delta(1,i,1),log10(kern_Y_min)-2,'.','MarkerSize',6,'Color',map(1,:));
...
% plot(kern_delta(rate+1,i,1),log10(kern_Y_sse(rate+1,i,1))-2,'.','MarkerSize',6,'Color',map(7,:));
