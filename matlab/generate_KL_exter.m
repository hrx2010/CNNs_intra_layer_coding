function T = generate_KL_exter(archname,testsize,klttype)
    ...
    for l = 1:l_length
        ...
        for k = 1:1
            for j = 1:1 % only one transform per group
                switch klttype
                  case 'kkt'
                    covH = cov(reshape(permute(double(layer_weights),[4,3,5,1,2]),q*1,[])',1);
                    covX = squeeze(X_vars);
                  case 'klt'
                    covH = cov(reshape(permute(double(layer_weights),[4,3,5,1,2]),q*1,[])',1);
                    covX = eye(q*1);
                ...
                invcovX = inv(covX+covX'+0.01*eye(q*1)*eigs(covX+covX',1));
