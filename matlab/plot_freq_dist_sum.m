...
load([archname,'_freq_sum_',num2str(testsize)]);
load([archname,'_base_',num2str(8)]);

vega = vega10(10);
figure(1);
loglog(10.^[-6,0],10.^[-6,0],'Color',0.0*[1,1,1]);
hold on; 
loglog(mean(pred_freq_sum_Y_sse(1:end,:,:),3)/1000, ...
       mean(hist_freq_sum_Y_sse(1:end,:,:),3)/1000,'.','MarkerSize',12,'Color',vega(6,:));
xticks(10.^(-6:1:0));
yticks(10.^(-6:1:0));
...
figure(2);
semilogy(mean(hist_freq_sum_coded(1:end,:,:),3),mean(hist_freq_sum_Y_sse(1:end,:,:),3)/1000,'.-','MarkerSize',8,'Color',vega(6,:));
hold on;
...
figure(3);
plot(mean(hist_freq_sum_coded(1:end,:,:),3),mean(hist_freq_sum_Y_sse(1:end,:,:),3)/1000,'.-','MarkerSize',8,'Color',vega(6,:));
hold on;
