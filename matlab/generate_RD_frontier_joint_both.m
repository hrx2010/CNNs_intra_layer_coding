function generate_RD_frontier_joint_both(archname,tranname,testsize,inlayers,outlayer,strides)
...
for j = 1:maxsteps
    ...
    for l = inlayers
        ...
        quant_weights = reshape(permute(transform_inter(perm5(quants(l).Weights,quants(l),size(quant_vectors,1)),...
                                                        quant_vectors(:,:,:,1)),[1,2,3,5,4]),[1,1,h*w*p*g,q]);
...
save(sprintf('%s_%s_sum_%d_%d_%d_%s_joint_total',archname,tranname,testsize,inlayers(1),inlayers(end),outlayer),...
