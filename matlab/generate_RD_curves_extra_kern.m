function generate_RD_curves_extra_kern(archname,tranname,testsize,inlayers,outlayer,strides)
...
for l = inlayers
    ...
    layer_weights = reshape(basis_vectors(:,:,:,1)*reshape(permute(layer_weights,[1,2,4,5,3]),h*w*q*g,p),h*w*q*g,p);
    kern_delta{l} = zeros(maxrates,maxsteps,h*w*q*g)*NaN;
    kern_coded{l} = zeros(maxrates,maxsteps,h*w*q*g)*NaN;
    kern_W_sse{l} = zeros(maxrates,maxsteps,h*w*q*g)*NaN;
    kern_Y_sse{l} = zeros(maxrates,maxsteps,h*w*q*g)*NaN;
    kern_Y_top{l} = zeros(maxrates,maxsteps,h*w*q*g)*NaN;
    s = strides(l);
    for i = 1:s:h*w*q*g % iterate over the frequency bands
        rs = i:min(h*w*q*g,s+i-1);
