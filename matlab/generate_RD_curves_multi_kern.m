function generate_RD_curves_multi_kern(archname,tranname,testsize,inlayers,outlayer,strides)
...
for l = inlayers
    ...
    layer_weights = reshape(intra_vectors(:,:,:,1)*reshape(permute(layer_weights,[4,5,1,2,3]),h*w,p*g*q),h*w,p*g,q);

    [s,t] = deal(2*strides(l,1),2*strides(l,2));
    points = ceil([h*w/s,p*g/t]);
        ...
        for c = 1:t:p*g
            ...
            for k = 1:maxrates %number of bits
                ...
                for j = 1:maxsteps
                    ...
                    coded = B*(length(rs)*length(cs)*1*1*q);
                ...
                disp(sprintf('%s %s | layer: %03d/%03d, band: %04d/%04d, scale: %+6.2f, delta: %+6.2f, ymse: %5.2e, wmse: %5.2e, top1: %5.2f, rate: %2.0f, time: %5.2fs', ...
                             ...
                             100*mean(kern_Y_top{l}(k,j,ri,ci)), coded/(length(rs)*length(cs)*1*1*q), sec));
            end
            save(sprintf('%s_%s_val_%d_%d_%d_%s_multi_kern',archname,tranname,testsize,l,l,outlayer),...
