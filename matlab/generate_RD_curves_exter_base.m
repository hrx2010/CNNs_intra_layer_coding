function generate_RD_curves_exter_base(archname,tranname,testsize,inlayers,outlayer,strides)
...
for l = inlayers
    ...
    base_delta{l} = zeros(maxrates,maxsteps,q*1)*NaN;
    base_coded{l} = zeros(maxrates,maxsteps,q*1)*NaN;
    base_W_sse{l} = zeros(maxrates,maxsteps,q*1)*NaN;
    base_Y_sse{l} = zeros(maxrates,maxsteps,q*1)*NaN;
    base_Y_top{l} = zeros(maxrates,maxsteps,q*1)*NaN;
    s = strides(l);
    for i = 1:s:q*1 % iterate over the frequency bands
        rs = i:min(q*1,s+i-1);
