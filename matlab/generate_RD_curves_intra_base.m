function generate_RD_curves_intra_base(archname,tranname,testsize,inlayers,outlayer,strides)
...
for l = inlayers
    basis_vectors = gettrans([tranname,'_50000_intra'],archname,l);
    ...
    base_delta{l} = zeros(maxrates,maxsteps,h*w)*NaN;
    base_coded{l} = zeros(maxrates,maxsteps,h*w)*NaN;
    base_W_sse{l} = zeros(maxrates,maxsteps,h*w)*NaN;
    base_Y_sse{l} = zeros(maxrates,maxsteps,h*w)*NaN;
    base_Y_top{l} = zeros(maxrates,maxsteps,h*w)*NaN;
    s = strides(l);
    for i = 1:s:h*w % iterate over the frequency bands
        ...
        for k = 1:maxrates %number of bits
            ...
            for j = 1:maxsteps
                ...
                quant = layers(l);
                quant.Weights = permute(reshape(quant_vectors(:,:,:,2)*layer_weights,h,w,p,g,q),[1,2,3,5,4]);
                coded = B*(s*h*w*1);
            ...
            disp(sprintf('%s %s | layer: %03d/%03d, band: %04d/%04d, scale: %+6.2f, delta: %+6.2f, ymse: %5.2e, wmse: %5.2e, top1: %4.1f, rate: %5.2e, time: %5.2fs', ...
                         archname, tranname, l, l_length, i, h*w, scale, delta, mean_Y_sse, mean_W_sse,...
                         100*mean(base_Y_top{l}(k,j,i)), coded/(s*h*w*1), sec));
        end
    end
    save(sprintf('%s_%s_val_%d_%d_%d_%s_intra_base',archname,tranname,testsize,l,l,outlayer),...
