...
for l in range(18,19):#0,len(layers)):
    with torch.no_grad():
        ...
        for i in range(0,1):#trans_weights.shape[0],s):
            ...
            for b in range(0,maxrates):
                ...
                for j in range(0,maxsteps):
                    sec = time.time()
                    delta = start + 0.25*j
                    coded = len(rs)*trans_weights.shape[1]
        ...
        io.savemat(('%s_%s_val_%03d_%04d_output_%s_kern_full.mat' % (archname,tranname,l,testsize,trantype)),\
