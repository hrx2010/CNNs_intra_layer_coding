...
for l in range(0,len(layers)):
    with torch.no_grad():
        ...
        layer_weight = layers[l].conv3.weight.clone()
        for i in range(0,m,s):
            ...
            for b in range(0,maxrates):
                ...
                for k in range(0,maxsteps):
                    ...
                    layers[l].conv3.weight[:] = quant_weight[:]
        ...
        layers[l].conv3.weight[:] = layer_weight[:]

        io.savemat(('%s_%s_val_%03d_%04d_output_%s_base3.mat' % (archname,tranname,l,testsize,'2d')),\
