        ...
        for i in range(0,trans_weights.size(1),s):
            ...
            for b in range(0,maxrates):
                ...
                for j in range(0,maxsteps):
                    sec = time.time()
                    delta = start + 0.25*j
                    coded = s*trans_weights.size(1)
                    quant_weights = trans_weights.clone()
                    quant_weights[rs] = quantize(quant_weights[rs],2**delta,b)
                    delta_weights = basis_vectors.flatten(2)[:,rs,1].mm(quant_weights[rs]-trans_weights[rs])
                    ...
                    if mean_Y_sse > last_Y_sse and \
                       ...
                       b == 0:
                        ...
                        print('%s %s | layer: %03d/%03d, band %04d/%04d, delta: %+6.2f, '
                              ...
                              % (archname, tranname, l, len(layers), i, trans_weights.size(0),\
