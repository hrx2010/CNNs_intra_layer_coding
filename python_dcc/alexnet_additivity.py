...
quantization_bit = int(sys.argv[2])
...
for i in range(num_conv_layers):
	...
	for t in range(max_steps):
		...
		quantized_weights = quantize(quantized_weights , np.power(2 , delta) , quantization_bit) #B
...
for i in range(num_conv_layers):
	...
	quantized_weights = np.array(all_weights_values_original[i])
	quantized_weights = quantize(quantized_weights , np.power(2 , delta) , quantization_bit)
	sess.run(node_weights.assign(quantized_weights))
...
file_results = open(('alexnet_additivity_%d_bits.txt' % (quantization_bit)), "a+")

file_results.write("%d %d %.16f %.16f\n" % (quantization_bit , quantization_layers , estimated_output_error , true_output_error))

file_results.close()

print('%d %d %.16f %.16f' % (quantization_bit , quantization_layers , estimated_output_error , true_output_error))
